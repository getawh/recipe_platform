...
if (isset($_POST['submit'])) {
  ...
  if ($result) {
    echo '<script> alert("Category Successfuly added!"); window.location.href = "view_category.php"; </script>';
  } else {
    echo '<script> alert("Cannot add category!"); </script>';
...
<body>
  <nav class="flex  py-3 z-20 justify-between rounded-md bg-opacity-80 backdrop-blur-md sticky top-2 bg-white px-7">
    ...
    <ul class="flex justify-between space-x-6   items-center">
      ...
      <a href="admin_panel.php" class="bg-green-600 px-4 py-1 text-sm font-bold text-white rounded hover:bg-green-500">Manage User</a>
      <a href="recipes_view.php" class="bg-green-600 px-4 py-1 text-sm font-bold text-white rounded hover:bg-green-500">Manage Recipe</a>
  ...
  <div class="container row d-flex flex-row justify-content mb-5 " style="justify-content:center; margin-top:3em;">
    <div class="admin-form ">
      <form action="" method="POST">
        ...
        <button type="submit" class="bg-green-500 my-2 hover:bg-green-600 text-white font-medium p-3 px-4 rounded-md" name="submit">Add</button>
      </form>
    </div>
  </div>
  </div>
